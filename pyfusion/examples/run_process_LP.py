...
_var_defaults="""
dev_name='W7X'
shot_list=[[20160309, sh] for sh in [41,42,43]]  # [13,41,42,43]] 13:many start later
shot_list=[[20160308, 27]]
shot_list=[]
shot_list.extend([[20160309, sh] for sh in [44,49,50,51,52,45,46]])
shot_list=[[20160309, 52]]
shot_list.extend([[20160308, 32], [20160309, 35]])
shot_list.extend([[20160310, sh] for sh in [38, 39]])
replace_kw={}
...
sweep_sig = 'W7X_L5UALL'  # OK, but if 53 is corrupted, it can affect 57 - so use L57_U in this case
seglist=[3,7]
time_range=None
"""
...
for dateshot in shot_list:
    for seg in seglist:
        try:
            print(seglist, seg, lpdiag.format(s=seg))
            LP = Langmuir_data(dateshot, lpdiag.format(s=seg),sweep_sig, time_range=time_range)
            ...
            pyfusion.logger.error('run_process_LP failed reading shot {shot}, segment {seg} \n{r}'
                                  .format(shot=dateshot, seg=seg, r=reason.__repr__()))
