...
def decimate(data, limit=None, fraction=None):
    ...
    if fraction is not None:
        if fraction > 1:
            raise ValueError('fraction ({f}) must be < 1'
                             .format(f=fraction))
...
def his(xa, tabs=False, sort=-1, dfmt=None, total=True, maxsort=None):
    ...
    if sort != 0:
        ...
        if maxsort is not None:
            ...
            ii = ii[-_maxsort:]
        if sort > 0:
            ii = ii[::-1]  # reverse so that greatest is first
    ...
    if total:
        print(' Total: {num:10d}  {pc:10.2f}%'.format(num=len(nxarr), pc=100))
