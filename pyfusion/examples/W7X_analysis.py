...
ax=subplot(1,1,1)
ax.plot(array(tresults).T[0],1/(0.6*q*A)*sqrt(mi/(q))/1e18*array(tresults).T[3]/sqrt(array(tresults).T[1]),label=r'~$n_e^{18}$')
ylim(0,8)
ax2=ax.twinx()
...
title('{c} {s}'.format(c=get_shot_info(*shot_number).strip(),s=shot_number))
show()
