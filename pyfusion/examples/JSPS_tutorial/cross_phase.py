...
see pyfusion/examples for a simpler version
...
for (i,pair) in enumerate(pairs[strtpair:]): 
    pl.plot(freq,coh[pair],label='dph'+str(pair),color=colorset[i],linewidth=lw)
    pl.plot(freq,cp[pair],color=colorset[i],linestyle=['--','-'][i==0 and strtpair==0],
            linewidth=lw)
    
pl.setp(ax2, ylim=(-4,4), ylabel = 'phase difference (rad) || coherence')
...
if nr>1: pl.setp(ax2, ylim=(-14.5,-12.5), ylabel='phase (rad)')
pl.setp(ax2.get_yticklabels()[-1],visible=False)
...
pl.setp(ax2, xlabel=str('frequency ({xlab})'.format(xlab=xlab)), xlim = xl)   
