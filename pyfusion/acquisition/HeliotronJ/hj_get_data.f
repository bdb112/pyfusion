c * Beware column 72--- should make this free format
...
C * See test examples at the end of fetch.py
     ...
     $     opt, ierror, isample, outname, outdata,n)
       implicit real (a-h,o-z)
c *** Note that the array size (n) is an argument generated by f2py 
       integer verbose, opt
       real outdata(n)
cf2py  intent(in,out,copy) :: outdata
cf2py  intent(in,out,copy) :: paneldt  ! not tested yet bdb
cxxf2py intent(callback) getdata  ! doesn't make sense Dave?
cf2py  intent(in, out) :: ierror
c Note that in order to put returned data into accessible memory
c variables that are intended for output only must also be input - see isample
cf2py  intent(in, out) :: isample
...
c looks like chname is a null terminated string, need to strip in fortran      
     ...
     +        // trim(chname) 
         if (opt .le. 0) then
            print * , ', will write to file ' // trim(outname)
         else
...
c *** check error ***
     ...
     $     write(6,*) 'Heliotron data found',isample,'samples'
c
c *** write data ***
      if (opt .le. 0) then
     ...
     $     // trim(outname)
      ...
      if (verbose .gt. 3) print *,'outdata(3) before copy', outdata(3)
      do 300 jt=1,isample
...
c     f2py returns all 'out' variables in a list, not tocuhing the copies
c       in the arg list
      if (verbose .gt. 3) 
     $      print *,'outdata(3), tmpary(1)',outdata(3),tmpary(1)
c      common /data/ iswch
c      print*, "iswch=",iswch
c
c *** stop program ***
