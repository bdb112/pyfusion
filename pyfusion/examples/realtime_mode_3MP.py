...
if reread:
# This seems to work only with one processor.
  tf=[0,.02,.04]
  tt=[.02,.04,.06]
  cmds,pipes = [],[]
  for i in range(len(tf)):
    cmds.append('python pyfusion/examples/gen_fs_bands.py n_samples=None df=1e3  max_bands=3 dev_name=H1Local shot_range=[{sht}] diag_name=H1ToroidalAxial overlap=2.5 exception=Exception debug=0   n_samples=None seg_dt=0.0005 time_range=[{tf},{tt}] separate=1 info=2 method="rms"'.format(sht=sht,tf=tf[i],tt=tt[i]))
    pipes.append(subprocess.Popen(cmds[i],  shell=True, stdout=subprocess.PIPE,
                                  stderr=subprocess.PIPE))
    print('waiting for prep_fs') 
    
  for i in range(len(tf)):
    (resp,err) = pipes[i].communicate()

    fname = 'PF2_150730{sht}p{i}s'.format(sht=sht,i=i)
    print(err)
