...
nfname = '/data/databases/W7X/LP/limiter_component_474.nodes.bz2'
i, x, y, z = np.loadtxt(nfname).T
efname = '/data/databases/W7X/LP/limiter_component_474.elements.bz2'
...
#  tried to visualize the Z=0.2 plane but we need +/- 0.02 becuse of tilt
plt.plot(X[wY0], Z[wY0], '+')
zu, inds = np.unique(z[wY0], return_index=1)  # remove the 4x duplication
xu = X[wY0][inds]
plt.plot(xu, zu, '+')
...
axdR.plot(Y[wZ0[wq]], X[wZ0[wq]]-5.6686, '.', label='limiter_profile')
axdR.set_aspect('equal')
axdR.set_ylabel('major axis relative to limiter tip')
...
json.dump(dict(rlim=xu.tolist(), zlim=zu.tolist(), shape_poly=pfit.tolist()), fh)
